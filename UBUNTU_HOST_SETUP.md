...
## ⚠ 인증 없음 + 0.0.0.0 바인딩 — 내부망 전체에 노출됨

이 에이전트는 **인증을 구현하지 않았고**, `winagent.service`는 `--host 0.0.0.0`으로
바인딩되어 있습니다. 즉 **같은 네트워크의 누구든 이 IP:8000으로 사용자/그룹
생성·삭제, ACL 변경, 폴더 삭제를 호출할 수 있습니다.** 방화벽/네트워크 분리로
신뢰된 환경에서만 접근 가능하도록 막아두세요. 운영 환경으로 넘어갈 경우
최소한의 API 키 검증이라도 추가하는 것을 강력히 권장합니다.

## 인증 관련 주의
이 에이전트는 **인증을 의도적으로 생략**했습니다 — 신뢰된 내부망에서만
호출되는 전제입니다. 외부(인터넷)에 노출하지 말 것.
